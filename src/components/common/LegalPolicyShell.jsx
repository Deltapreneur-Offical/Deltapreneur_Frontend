    ...
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <section className="relative min-h-screen w-full overflow-hidden bg-gradient-to-b from-slate-50 via-white to-slate-50 text-slate-900">
        <div className="pointer-events-none absolute inset-0">
          <motion.div
            className="absolute -top-24 left-1/4 h-72 sm:h-80 md:h-96 w-72 sm:w-80 md:w-96 rounded-full bg-orange-200/40 blur-3xl"
            animate={{ x: [0, 40, -20, 0], y: [0, -20, 20, 0], scale: [1, 1.1, 0.95, 1] }}
            transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }}
          />
          <motion.div
            className="absolute -bottom-24 right-1/4 h-72 sm:h-80 md:h-96 w-72 sm:w-80 md:w-96 rounded-full bg-teal-200/35 blur-3xl"
        ...
        <div className="relative mx-auto max-w-7xl px-2 sm:px-4 md:px-6 lg:px-8">
          ...
          >
            <div className="inline-flex items-center gap-2 rounded-full border border-orange-200/80 bg-orange-50 px-4 py-2 mb-5 shadow-sm">
              <BadgeIcon className="h-4 w-4 text-orange-600" />
              <span className="text-sm font-semibold text-orange-800">{badge}</span>
            </div>
                  ...
                  {sections.map((s) => (
                    <li key={s.id}>
                      <button
                        ...
                        className="group flex w-full items-center justify-between rounded-xl px-3 py-2 text-xs sm:text-sm text-slate-600 hover:text-orange-700 hover:bg-orange-50 transition"
                      >
                        <span className="truncate">{s.title}</span>
                        <ChevronRight className="h-4 w-4 text-slate-400 opacity-80 group-hover:text-orange-600 group-hover:opacity-100 transition" />
                      </button>
                    </li>
                  ))}
                </ul>
    ...
    <div className="grid grid-cols-1 gap-4 mt-2">
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
        ...
        <p className="text-sm text-slate-600 mt-2 flex items-start gap-1.5">
          <MapPin className="h-3.5 w-3.5 text-teal-700 mt-0.5 shrink-0" />
          <span>
            Address:{' '}
        ...
        <p className="text-sm text-slate-600 mt-2">
          Contact page:{' '}
          <a href="/contact" className="font-medium text-teal-700 hover:underline">
