...
const INTERNAL_PATTERN =
  /openrouter|openprovider|open provider|resellerclub|reseller club|sqlalchemy|psycopg|operationalerror|traceback|econnrefused|enotfound|network error|failed to fetch|axioserror|httpx|aiohttp|exception:|\.py["']|line \d+:|api[_ ]?key|secret[_ ]?missing|not configured|database_unavailable|database is not reachable|database is unavailable|cobrother_backend|deltapreneur_backend|database_url|127\.0\.0\.1:5433|connection refused|could not connect to|postgres|pg_conn|rds tunnel|run_.*\.ps1|port 5433|add credits at|openrouter\.ai|invalid openrouter|internal server error|unexpected error occurred|hostname.*not known|ssl.*certificate|certificate verify failed|legacy_resellerclub/i;
