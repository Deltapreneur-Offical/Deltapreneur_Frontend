...
/**
 * Pre-written Deltapreneur share message with the CURRENT domain state.
 *
...
export function buildShareMessage({ domain, shareUrl, availability }) {
  ...
  const lines = [`🚀 Check out ${domain} on Deltapreneur!`];
...
export function buildSocialLinks(shareUrl, domain, availability) {
  ...
  const subject = encodeURIComponent(`Check out ${domain} on Deltapreneur!`);
            ...
            <>
              ...
              <div className="mt-1.5 rounded-lg border border-red-100 bg-red-50 px-2.5 py-1.5">
                <p className="text-[11px] font-medium text-red-600">
                  Note: Rewards are available to active Deltapreneur users only.
                </p>
              </div>
            </>
          )}
