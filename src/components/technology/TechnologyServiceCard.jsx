...
export default function TechnologyServiceCard({ service, compact = false, homeLayout = false }) {
  ...
  return (
    ...
    >
      {/* Top Accent Glow */}
      <div className="tech-service-card__accent-glow absolute -top-12 -right-12 h-32 w-32 rounded-full bg-gradient-to-br from-sky-500/10 to-blue-600/10 blur-xl transition-all duration-75 group-hover:scale-150 group-hover:from-orange-600/20 group-hover:to-orange-800/20" />

      {homeLayout ? (
        /* ── Reference UI card layout ── */
        <>
          {/* Icon + badges row */}
          <div className="tech-service-card__ref-header">
            <div className="tech-service-card__ref-icon">
              <IconComponent className="h-5 w-5" />
            </div>
            <div className="tech-service-card__ref-badges">
              <span className="tech-service-card__ref-badge tech-service-card__ref-badge--cat">
          ...
          {/* Title */}
          <h3 className="tech-service-card__ref-title">
            {service.name}
          </h3>

          {/* Description */}
          <p className="tech-service-card__ref-desc">
            {techT(t, `techDesc_${service.slug}_`, service.short_description)}
          </p>

          {/* Footer: price + dark CTA arrow */}
          <div className="tech-service-card__footer tech-service-card__ref-footer">
            <div className="tech-service-card__ref-price-block">
              <span className="tech-service-card__ref-price-label">
                {t('commonStartingAt', { defaultValue: 'Starting at' })}
              </span>
              <div className="tech-service-card__ref-price-row">
                <span className="tech-service-card__ref-price-amount">
                  {formatTechPrice(service.starting_price || 15)}
                </span>
                <span className="tech-service-card__ref-price-suffix">
                  /{t('commonMo', { defaultValue: 'mo' })}
                </span>
              </div>
            </div>
            <button
              ...
              className="tech-service-card__ref-cta"
            >
        ...
        /* ── Standard full-page card layout ── */
        <div>
          {/* Header Row */}
          <div className={`flex items-start justify-between gap-2 min-w-0 mb-4`}>
